...
import { Link, Lock, Unlock, Copy } from "lucide-react";
...
export default function URLTool() {
  ...
  const handleEncode = () => {
    try {
      ...
      setResult("Erro ao codificar URL");
    }
  };
  ...
  const handleDecode = () => {
    try {
      ...
      setResult("Erro ao decodificar URL: Entrada inválida");
    }
  };
  ...
  return (
    <ToolCard
      ...
      icon={<Link className="text-primary w-6 h-6" />}
    >
      <div className="space-y-4">
        <Textarea
          value={input}
          onChange={(e) => setInput(e.target.value)}
          placeholder="Digite a URL para codificar/decodificar"
